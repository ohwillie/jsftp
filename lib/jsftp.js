...
Ftp.prototype._createSocket = function(port, host, firstAction = NOOP) {
  ...
  if (this.ssl) {
    this.socket = tlsConnect(port, host, this.sslOptions);
    this.socket.on("secureConnect", () => {
      this.runCommand("prot p", () => {
        this.emit("connect");
      });
    });
  } else {
    this.socket = createConnection(port, host, firstAction || NOOP);
...
Ftp.prototype.getPasvSocket = function(callback = NOOP) {
  ...
  this.execute("pasv", (err, res) => {
    ...
    let socket;
    if (this.ssl) {
      socket = tlsConnect(options.port, options.host, this.sslOptions);
      ...
      socket = (this._pasvSocket = createConnection(options));
      socket.once("connect", () => {
        this._pasvSocket = socket;
      });
    }
